...
public class RobotContainer {
    ...
    private void configureButtonBindings() {
        ...
        rightTrigger.onTrue(new SwerveDriveAlignToHub().andThen(new SwerveDriveXMode()).andThen(new ShooterSetShoot()).andThen(new HandoffSetForward()).alongWith(new FeederSetForward(), new IntakeAgitateOnce().repeatedly()));
        // Top Left Paddle
        driver.a().onTrue(new IntakeSetIdle());
        driver.povUp().onTrue(new SwerveDriveResetRotation());
        driver.rightBumper().onTrue(new SwerveDriveAlignToFerryZone().andThen(new SwerveDriveXMode()).andThen(new ShooterSetFerry()).andThen(new HandoffSetForward()).alongWith(new FeederSetForward(), new IntakeAgitateOnce().repeatedly()));
        // Bottom Right Paddle
        // Manual shooting possibly from in front of the hub
        driver.y().onTrue(new SwerveDriveXMode().andThen(new ShooterSetManual()).andThen(new HandoffSetForward()).alongWith(new FeederSetForward(), new IntakeAgitateOnce().repeatedly()));
        // Top Right Paddle

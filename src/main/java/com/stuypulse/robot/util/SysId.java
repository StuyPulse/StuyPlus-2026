 ...
 *
 *
 * <h2>A class that handles the Mechanism logging and unit conversions required to create a SysId
 * routine</h2>
 */
public class SysId {
     ...
     *
     * @param rampRate - Measured in volts per second, the voltage ramp rate used for quasistatic test
     *     routines.
     ...
     * @param subsystemInstance - The subsystem containing the motor(s) that is (or are) being
     *     characterized.
     */
     ...
     * <h4>Creates a SysId routine from the parameters</h4>
     *
     * <p>Uses WPILib unit classes so inputs can be in any unit.
     *
                ...
                new SysIdRoutine.Mechanism(
                        ...
                        state -> {
                            SignalLogger.writeDouble(
                                    subsystemName + " Position", positionSupplier.get().in(Rotations));
                            SignalLogger.writeDouble(
                                    subsystemName + " Velocity", velocitySupplier.get().in(RotationsPerSecond));

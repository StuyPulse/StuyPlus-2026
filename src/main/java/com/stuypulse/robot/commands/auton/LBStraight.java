...
public class LBStraight extends SequentialCommandGroup {
    ...
    public LBStraight(PathPlannerPath... paths) {
        addCommands(VisionCommands.disable(), new SwerveResetPose(paths[0].getStartingHolonomicPose().get()), // new
                                                                                                             // IntakeSetIntake(),
                CommandSwerveDrivetrain.getInstance().followPathCommand(paths[0]),
                CommandSwerveDrivetrain.getInstance().followPathCommand(paths[1]), IntakeCommands.setHomingDown());
    }
}

 ...
 *
 * <p>The actual class that performs the AST parsing and replacement of <code>SmartDashboard</code>
...
*
* <p>It loops through every {@link MethodCallExpr} to check for a <code>SmartDashboard</code>
* call with the methods <code>putNumber</code>, <code>putBoolean</code>, or <code>putString
* </code>. If so, it replaces the scope with <code>DogLog</code> and changes the method name to
* <code>log</code>.
*
* <p>Sets variable <code>fileChanged</code> to check if any changes were made to the file to
...
*
* <p>Applies the <code>SmartDashboard</code> to <code>DogLog</code> replacement to the given
* {@link CompilationUnit}.
*
* <p>If the <code>fileChanged</code> flag is true, it checks if the files already has an import
* for <code>DogLog</code>. If not, it adds the import statement. If the flag is false, it does
...
public static void applyReplacement(CompilationUnit parsed) {
	...
	boolean hasDogLogImport =
		parsed.getImports().stream().anyMatch(i -> i.getNameAsString().equals(doglogImport));

...
public class Main {
    ...
    public static void processFile(Path filePath) {
        try {
            ...
            System.out.println(
                    "Error processing file: " + filePath + ", will be skipped: " + errorReason);
        }
    }
    ...
    private static void configureDogLog() {
        try {
            ...
            parsed
                    .findFirst(
                            MethodDeclaration.class, method -> method.getNameAsString().equals("robotInit"))
                    .ifPresent(
                            method -> method.getBody().ifPresent(body -> body.addStatement(doglogConfigCall)));
